...
export default function DiscoverLeadership() {
  const { notify } = useNotifications();
  const { isFetching, isError, error } = useGetCourseQuery(undefined);
  const data = useSelector(getFilteredCourses("DL"));
  ...
  const leadershiptype = Course.Discovering_Leadership;
  ...
  const breadcrumbLinks = [
    ...
    { to: null, label: "DL courses", main: true },
  ];
